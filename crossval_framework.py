...
def predict_cluster_labels(x, n_params, k_clusters=7):
    svd_model = TruncatedSVD(algorithm='randomized', n_components=n_params)
    normalizer = Normalizer(copy=False)
    lsa = make_pipeline(svd_model, normalizer)
    x_svd = lsa.fit_transform(x)
    k_model = get_kmeans(x_svd, k_clusters=k_clusters)
    kmeans_fitted = k_model.fit(x_svd)
    labels = kmeans_fitted.labels_
...
def clusters_to_csv(labels, n):
    test_output = 'cluster_output/test_results_labels{}params_{}.csv'.format(n, get_time())
...
if __name__ == '__main__':
    ...
    get_iris_dataset = True     #get iris
    ...
    do_print_results = True    # print output to file
    ...
    if do_print_results:
        k_clusters = 3
        n_params = 4
        # x_labels = predict_cluster_labels(x, n_params, k_clusters)

        x_labels = KMeans(n_clusters=3).fit(x).labels_
        clusters_to_csv(x_labels, n_params)
